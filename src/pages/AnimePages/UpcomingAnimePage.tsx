...
const UpcomingAnimePage = () => {
    ...
    const getAnimeList=async()=>{
        setLoading(true);
        window.scrollTo(0,0);
        let result = await axios.get(`https://api.jikan.moe/v4/seasons/upcoming?page=${pageNumber}`);
  ...
  return (
    <div>
         <MainDiv>
            ...
            <Heading>
                    <span>Upcoming Anime Page {pageNumber}</span> Results
            </Heading>
            
            {Loading && <SearchResultsSkeleton movieName={'Upcoming Anime Results'} />}
            ...
            {!Loading && (
                        ...
                        <CardWrapper>
                            {animeList.map((item: any, index: any) => {
                                return (
                                    ...
                                    key ={item.id}
                                    >
                                        <Links to={`/animes/search/${item.title ?? item.title_english ?? item.title_japanese}`}>
                ...
                <div className='pagination'>
                    <Link onClick={()=>{
                        ...
                        return;
                        }}  to={ pageNumber > 1 ?  `/animes/upcoming&page=${pageNumber-1}` : `/animes/upcoming&page=${pageNumber}`}>
                        {"PREV"}
                    </Link>

                    <span>
                        {
                        `${pageNumber} OF ${pageInfo.last_visible_page} `
                        }
 
                    </span>
                   

                    <Link onClick={()=>setPageNumber(pageNumber+1)}  to={`/animes/upcoming&page=${pageNumber+1}`}>
                        {"NEXT"}
                    </Link>
                </div>
        </MainDiv>
