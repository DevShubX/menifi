...
const PopularMoviesPage = () => {
    ...
    const getPopMovies = async () => {
        setLoading(true);
        let result = await axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=${pageNumber}`);
    ...
    return (
        <div>
            <MainDiv>
                ...
                {loading && <SearchResultsSkeleton movieName={'Popular Movies Result'} />}
                {!loading && (
                    <>
                        <CardWrapper>
                            {PopularMovies.results.map((item: any, index: any) => (
