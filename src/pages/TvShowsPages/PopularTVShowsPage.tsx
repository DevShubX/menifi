...
const PopularTVShowsPage = () => {
    ...
    let [PopularTvShows,setPopularTvShows] = useState<any>({});
    ...
    const getPopTvShows =async()=>{
        setLoading(true);
        let pop_tv = await axios.get(`https://api.themoviedb.org/3/tv/popular?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=${pageNumber}`);
        setPopularTvShows(pop_tv.data);
        setLoading(false);
    }
  ...
  return (
    <div>
        <MainDiv>
            ...
            <Heading>
                <span>Popular TV Shows</span> Results
            </Heading>
            ...
            {!loading && (
                <>
                <CardWrapper>
                     {PopularTvShows.results.map((item:any,index:any)=>(
                        <motion.div
            ...
            <ReactPaginate
                ...
                pageCount={PopularTvShows.total_pages}
                nextLabel="Next >"
