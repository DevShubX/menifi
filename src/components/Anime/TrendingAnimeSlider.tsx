...
const TrendingAnimeSlider = () => {
    ...
    const getTrendingAnimes = async () => {
        let result = await axios.get(`https://redux-api-wine.vercel.app/api/trending?page=1`)
        setTrendingAnime(result.data.data.Page.media);
        setLoading(false);
    }
    return (
        <div>
            <MainDiv>
                ...
                {!loading && (
                    ...
                    >
                        {trendingAnime.map((item: any, index: any) => (
                            <SwiperSlide key={item.id}>
                                <Wrapper>
                                    <Link to={"/animes/search/" + (item.title.english !== null || undefined || "" ? item.title.english : item.title.romaji !== null || undefined || "" ? item.title.romaji : item.title.userPreferred)}>
