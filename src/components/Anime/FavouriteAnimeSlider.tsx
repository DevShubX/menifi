...
const FavouriteAnimeSlider = () => {
    const [favouriteAnime, setFavouriteAnime] = useState<any>([]);
    ...
    const getFavouriteAnime = async () => {
        setLoading(true);
        let result = await axios.get(`https://redux-api-wine.vercel.app/api/favourite?page=1&count=18`);
        setFavouriteAnime(result.data.data.Page.media);
        setLoading(false)
    };
    return (
        <div>
            <MainDiv>
                ...
                {!loading && (
                    ...
                    >
                        {favouriteAnime.map((item: any, index: any) => (item.coverImage.large !== null || undefined || "") && (
...
export default FavouriteAnimeSlider